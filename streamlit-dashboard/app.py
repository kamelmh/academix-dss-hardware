...
# Page config
st.set_page_config(
    page_title="Academix DSS - Tableau de Bord",
    page_icon="📊",
    layout="wide"
)
...
# Sample data - Quincaillerie inventory
@st.cache_data
...
with col2:
    st.metric(
        label="💰 Valeur Stock",
        value=f"{products['Valeur_Stock'].sum():,.0f} DZD",
        delta="+5.2%"
    )
...
with col3:
    st.metric(
        label="⚠️ Alertes",
        value=f"{len(products[products['Statut'] != 'Normal'])}",
        delta="-3"
    )
...
with col4:
    st.metric(
        label="🔄 Rotation Moy.",
        value=f"{products['Rotation'].mean():.1f}",
        delta="+0.8"
    )
...
with col2:
    ...
    abc_data = products.groupby('ABC').agg({
        'ID': 'count',
        'Valeur_Stock': 'sum'
    }).reset_index()
    ...
    fig.add_trace(go.Bar(
        x=abc_data['Classe'],
        y=abc_data['Nb_Articles'],
        name='Nb Articles',
        marker_color='#3b82f6'
    ))
    fig.add_trace(go.Bar(
        x=abc_data['Classe'],
        y=abc_data['Valeur'] / 1000000,
        name='Valeur (M DZD)',
        marker_color='#10b981',
        yaxis='y2'
    ))
    fig.update_layout(
        ...
        yaxis2=dict(title='Valeur (M DZD)', overlaying='y', side='right'),
        barmode='group'
    )
    st.plotly_chart(fig, use_container_width=True)
...
with col2:
    ...
    fig = px.bar(top_rotation, x='Nom', y='Rotation', 
...
if len(alerts) > 0:
    for _, row in alerts.iterrows():
        status_color = "🔴" if row['Statut'] == 'Critique' else "🟡"
        st.write(f"{status_color} **{row['Nom']}** - Stock: {row['Stock_Actuel']} | Seuil: {row['Seuil_Alerte']} | Fournisseur: {row['Fournisseur']}")
else:
...
# Footer
st.divider()
st.caption("© 2026 Academix DSS - Système de Gestion de Stock pour Quincailleries")
st.caption("Contact: kamelmahi71@gmail.com | WhatsApp: +213 676 77 38 92")
